...
abstract class Model implements Listenable {
  ...
  final List<VoidCallback> _modifications = <VoidCallback>[];
  int _version = 0;
  ...
  void notifyListeners() {
    ...
    if (_microtaskVersion == _version) {
      ...
      scheduleMicrotask(() {
        ...
        for (final VoidCallback modification in _modifications) {
          modification();
        }
...
class ModelFinder<T extends Model> {
  ...
  T? of(BuildContext context, {bool rebuildOnChange = false}) {
    ...
    final _InheritedModel<T>? widget = rebuildOnChange
...
class _InheritedModel<T extends Model> extends InheritedWidget {
  _InheritedModel({super.key, required super.child, required this.model})
...
class ScopedModelDescendant<T extends Model> extends StatelessWidget {
  /// Constructor.
  const ScopedModelDescendant({super.key, required this.builder, required this.child});
